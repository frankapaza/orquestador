...
const SECTIONS = [
  ...
  },
  {
    ...
    description: 'IDENTIDAD por DOCUMENTO (DNI/RUC): un contacto = una persona, con VARIOS teléfonos y correos, que puede estar en VARIAS listas SIN duplicarse. El teléfono se guarda SEPARADO (phone nacional + phone_dial "+51" + phone_country "PE").',
    endpoints: [
      { method: 'GET', path: '/contacts?q=&page=1&limit=30', title: 'Buscar contactos (global, por documento)',
        query: [{ name: 'q', desc: 'Busca por DOCUMENTO, nombre, teléfono o correo (en todos los teléfonos/correos del contacto).' }, { name: 'page', desc: 'Página (default 1)' }, { name: 'limit', desc: 'Por página (default 30, máx 100)' }],
        description: 'Hub global: 1 fila por contacto (identidad por documento), con su teléfono/correo principal, cuántos tiene, y las listas a las que pertenece.',
        response: { contacts: [{ id: '...', document: '76011618', first_name: 'Jose', last_name: 'Chacpa', phone: { phone: '936109504', phone_dial: '+51', phone_country: 'PE' }, email: 'jose@correo.com', phone_count: 2, email_count: 1, lists: [{ id: '...', name: 'Cartera Enero' }] }], total: 1, page: 1 } },
      { method: 'POST', path: '/contacts', title: 'Crear / actualizar contacto (por documento)',
        body: { document: '76011618', first_name: 'Jose', last_name: 'Chacpa', phone: '936109504', phone_dial: '+51', phone_country: 'PE', email: 'jose@correo.com', list_ids: ['ID_LISTA'] },
        note: 'IDENTIDAD por DOCUMENTO (obligatorio). Si el documento YA existe se REUTILIZA el contacto y se le SUMAN el teléfono/correo nuevos (no se duplica). Un mismo número puede pertenecer a personas (documentos) distintas. list_ids opcional (a qué listas agregarlo). phone es el número nacional; phone_dial el código (+51). Si mandas phone con el código pegado (ej. 51936109504) más phone_country/phone_dial, el sistema lo separa.',
        response: { id: '...', document: '76011618', first_name: 'Jose', last_name: 'Chacpa' } },
      ...
      { method: 'POST', path: '/lists/:listId/contacts', title: 'Agregar contacto a una lista',
        params: [{ name: 'listId', desc: 'ID de la lista' }],
        body: { document: '76011618', first_name: 'Juan', last_name: 'Pérez', email: 'juan@empresa.com', phone: '910462070', phone_dial: '+51', phone_country: 'PE' },
        note: 'Igual que POST /contacts pero además lo agrega a esta lista. document identifica al contacto (si existe, se reutiliza). Debe incluir al menos email o phone. phone es el número NACIONAL; phone_dial el código (+51). Si envías phone con el código pegado (ej. 51910462070) más phone_country, se separa solo.',
        response: { id: '...', document: '76011618', first_name: 'Juan', email: 'juan@empresa.com', phone: '910462070', phone_dial: '+51', phone_country: 'PE' } },
  ...
  {
    id: 'campaigns', Icon: Megaphone, title: 'Campañas (integración cobranza)',
    description: 'FLUJO: (1) carga los destinatarios → obtienes un list_id [Excel: POST /campaigns/import-recipients · o contacto a contacto: POST /contacts con list_ids]; (2) crea la campaña apuntando a ese list_id (POST /campaigns); (3) envíala (POST /campaigns/:id/send); (4) sigue el progreso (GET /campaigns/:id). Canales: Email, WhatsApp, WhatsApp IA y SMS. Escucha message.sent / message.received con Webhooks para reflejar todo en tu sistema.',
    endpoints: [
      { method: 'POST', path: '/campaigns/import-recipients?channel=whatsapp&name=Cartera%20Enero', title: '1) Importar destinatarios (Excel/CSV) → crea lista',
        query: [{ name: 'channel', desc: 'whatsapp | sms | email — define si exige teléfono o correo' }, { name: 'name', desc: 'Nombre base de la lista que se crea' }, { name: 'assistant_id', desc: '(opcional) para campañas WhatsApp IA' }],
        description: 'multipart/form-data con el archivo en el campo "file". Columnas: documento (OBLIGATORIO), telefono (WA/SMS) o correo (Email), pais (opcional: "+51" o "PE"), nombre, apellido, y CUALQUIER columna extra queda como variable {{...}} para el mensaje (ej. monto, factura, fecha_vencimiento). Dedup por documento; al mismo documento en varias filas se le suman sus teléfonos.',
        note: 'Content-Type: multipart/form-data, campo "file". Devuelve el list_id para usar en POST /campaigns. skipped = filas descartadas (sin documento, teléfono/correo o país inválido) con su motivo.',
        response: { list_id: 'ID_LISTA', list_name: 'Cartera Enero — destinatarios', total: 120, columns: ['monto', 'factura', 'fecha_vencimiento'], variables_faltantes: [], skipped: 2, skipped_detail: [{ row: 5, value: '(sin documento)', reason: 'documento vacio' }] } },
      { method: 'POST', path: '/campaigns', title: '2) Crear campaña (WhatsApp / SMS)',
        body: { name: 'Recordatorio pago', channel: 'whatsapp', content_text: 'Hola {{nombre}}, tu factura {{factura}} por {{monto}} vence el {{fecha_vencimiento}}.', list_id: 'ID_LISTA', media_url: 'https://...opcional', settings: { send_to_all: true } },
        note: 'channel: "whatsapp" o "sms". Requiere content_text con variables {{...}} (columnas del Excel). Rota entre tus cuentas WA/SMS conectadas. settings.send_to_all = true → a TODOS los teléfonos del contacto; false → solo el principal. En WhatsApp puedes adjuntar media_url (+ media_caption).',
        response: { id: '...', name: 'Recordatorio pago', channel: 'whatsapp', status: 'draft' } },
      { method: 'POST', path: '/campaigns', title: '2) Crear campaña (WhatsApp IA — la IA conversa sola)',
        body: { name: 'Cobranza IA', channel: 'whatsapp', assistant_id: 'ID_ASISTENTE', list_id: 'ID_LISTA', settings: { send_to_all: true, wa_account_ids: ['ID_NUM_1', 'ID_NUM_2'] } },
        note: 'Envía el saludo del asistente (con las variables {{...}} del Excel) y, cuando el cliente RESPONDE, un modelo de IA conversa por ti. assistant_id + settings.wa_account_ids = números de WhatsApp vinculados a ese asistente (rota entre ellos). El asistente debe tener saludo configurado.',
        response: { id: '...', name: 'Cobranza IA', channel: 'whatsapp', assistant_id: 'ID_ASISTENTE', status: 'draft' } },
      { method: 'POST', path: '/campaigns', title: '2) Crear campaña (Email)',
        body: { name: 'Aviso cobranza', channel: 'email', subject: 'Recordatorio de pago — {{first_name}}', from_name: 'Cobranzas', html_content: '<p>Factura {{factura}} por {{monto}}</p>', strategy: 'smtp_own', list_id: 'ID_LISTA', settings: { send_to_all: false, track_opens: true } },
        note: 'channel: "email". Requiere subject, from_name y html_content. strategy: smtp_own | sendgrid | brevo | mailchimp (las externas requieren settings.integration_id). send_to_all = true → a TODOS los correos; false → solo el principal.',
        response: { id: '...', name: 'Aviso cobranza', channel: 'email', status: 'draft' } },
      { method: 'POST', path: '/campaigns/:id/send', title: '3) Enviar / encolar campaña',
        params: [{ name: 'id', desc: 'ID de la campaña' }],
        description: 'Encola la campaña. Genera un trabajo por cada DESTINO (un contacto con 3 teléfonos = 3 envíos si send_to_all está activo), con delays anti-baneo.',
        response: { message: 'Campaña encolada para envío', campaign_id: '...' } },
      { method: 'POST', path: '/campaigns/:id/pause', title: 'Pausar envío',
        params: [{ name: 'id', desc: 'ID de la campaña' }],
        response: { ok: true, status: 'paused' } },
      { method: 'POST', path: '/campaigns/:id/resume', title: 'Reanudar envío',
        params: [{ name: 'id', desc: 'ID de la campaña' }],
        response: { ok: true, status: 'sending' } },
      { method: 'GET', path: '/campaigns/:id', title: '4) Detalle + progreso (2 métricas)',
        params: [{ name: 'id', desc: 'ID de la campaña' }],
        description: 'Progreso en DOS métricas: por CONTACTOS (total_contacts / done_contacts — el cliente cuenta como hecho solo cuando TODOS sus teléfonos/correos se enviaron) y por DESTINOS (total_recipients / sent_count / failed_count). Estados: draft, scheduled, sending, paused, completed, failed.',
        response: { id: '...', name: 'Cobranza IA', channel: 'whatsapp', status: 'sending', total_recipients: 200, sent_count: 120, failed_count: 3, total_contacts: 150, done_contacts: 90 } },
      { method: 'GET', path: '/campaigns/:id/jobs?page=1&status=sent', title: 'Destinatarios (envíos, por destino)',
        params: [{ name: 'id', desc: 'ID de la campaña' }],
        query: [{ name: 'status', desc: 'sent | pending | failed | invalid | bounced | undelivered' }, { name: 'page', desc: 'Página' }],
        description: 'Un registro por DESTINO (teléfono/correo). delivery_status refleja el acuse de WhatsApp (sent / delivered / read).',
        response: { jobs: [{ id: '...', contact_id: '...', phone_number: '+51936109504', first_name: 'Jose', status: 'sent', sent_at: '...', delivery_status: 'delivered' }], total: 200, pages: 7 } },
      { method: 'GET', path: '/campaigns', title: 'Listar campañas',
        response: [{ id: '...', name: 'Cobranza IA', channel: 'whatsapp', status: 'completed', list_name: 'Cartera Enero', total_recipients: 200, sent_count: 197, total_contacts: 150, done_contacts: 150 }] },
    ],
  },
  {
    ...
    endpoints: [
      ...
      { method: 'POST', path: '/contacts/:id/send-email', title: 'Enviar correo individual',
        params: [{ name: 'id', desc: 'ID del contacto' }],
        body: { subject: 'Hola {{first_name}}', from_name: 'Equipo de ventas', html_content: '<p>Tu mensaje aquí</p>', to: 'juan.trabajo@empresa.com', account_id: 'ID_CUENTA_EMAIL', cc: ['copia@empresa.com'], bcc: ['oculto@empresa.com'] },
        note: 'Variables: {{first_name}}, {{last_name}}, {{email}}. "to" opcional (si se omite usa el correo PRINCIPAL). account_id opcional: cuenta SMTP emisora (de GET /email/accounts); si se omite se auto-selecciona. CC = copia, BCC = copia oculta (arrays). El envío queda en la vista 360 del contacto.',
        response: { ok: true, message_id: '<id@whaxia.com>', to: 'juan.trabajo@empresa.com' } },
      { method: 'GET', path: '/email/accounts', title: 'Listar cuentas de correo (account_id)',
        note: 'Cuentas SMTP activas del cliente. El "id" de cada una es el account_id para elegir DESDE qué correo enviar en /email/send y send-email.',
        response: [{ id: '...', email: 'ventas@tudominio.com', domain: 'tudominio.com', smtp_host: 'mail.tudominio.com' }] },
      { method: 'POST', path: '/email/send', title: 'Enviar correo desde cuenta específica (CRM)',
        body: { account_id: 'ID_CUENTA_EMAIL', to: 'cliente@correo.com', subject: 'Asunto', html_content: '<p>Contenido</p>', from_name: 'Cobranzas', cc: ['copia@empresa.com'], bcc: ['oculto@empresa.com'] },
        note: 'Envío 1:1 DESDE la cuenta SMTP indicada (account_id) hacia cualquier destino (no necesita ser contacto). Para integrar desde CRMs externos (ej. MCOB). Si el destino coincide con un contacto, queda en su 360.',
        response: { ok: true, message_id: '<id@tudominio.com>', to: 'cliente@correo.com', from: 'ventas@tudominio.com' } },
    ],
  },
  {
